...
myformula=pm10~f(spatial,model=myspde)
...
Another (more general) way to define spatial structure is to define the \textbf{variogram} $Var(Y(x) - Y(y)) = 2 \gamma(x,y)$ (with $\gamma()$ the \textbf{semivariogram}) and its stationary version $2\gamma(h) = 2 (\mathcal{C}(0) - \mathcal{C}(h));h\in\mathbb{R}$.
...
for (j in 1:4){
  plot(res$marginals.hyper[[j]][,1],res$marginals.hyper[[j]][,2], type = 'l', 
       xlab = names(res$marginals.hyper)[j], ylab = 'Density',cex=26)
...
\textbf{Goal:} predict $Y_0 = Y(x_0)$ from observations
$\mathbf{Y}=(Y_1,\dots,Y_n)$ at locations $(x_1,\dots,x_n)$

\vspace{0.3cm}

\pause

\textbf{Linear unbiased predictor}
\[
\hat Y_0 = \sum_{i=1}^n \ell_i Y_i,
\qquad
\sum_{i=1}^n \ell_i = 1
\]
...
\textbf{Optimality criterion (MMSE)}
\[
\min_{\ell_i}
E\!\left[\left(Y_0 - \hat Y_0\right)^2\right]
\]
...
\textbf{Geostatistical formulation}

Let $a_0=1$, $a_i=-\ell_i$ $\Rightarrow$ $\sum_{i=0}^n a_i=0$

\[
E\!\left[\sum_{i=0}^n a_i Y_i\right]^2
=
- \sum_{i,j} a_i a_j \, \gamma(\|x_i-x_j\|)
\]
...
\begin{center}
\textbf{Result:} solving this constrained quadratic problem gives the
\textbf{kriging weights}
\end{center}
...
- **Joint species distribution models, factor models and ecosystem forecasting**

Thorson, J. T., Arimitsu, M. L., Barnett, L. A., Cheng, W., Eisner, L. B., Haynie, A. C., ... & Siddon, E. C. (2021). Forecasting community reassembly using climate‐linked spatio‐temporal ecosystem models. Ecography, 44(4), 612-625.
